...
In essence it is a microdeduplicator using Matches/Blocks/Granularities from 8..256 at step 8, encoding them in 1+(1..8) bytes, that is, using 2^(1x8)=256B, 2^(2x8)=64KB,... 2^(8x8)=16EiB windows for backwards lookups, so it is kinda non-entropy compressor.
