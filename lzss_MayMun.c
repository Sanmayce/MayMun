// lzss_MayMun version 5forte (fixed the mmap so that it really is on file), written by Grok and Kaze (sanmayce@sanmayce.com), 4138 Lines-Of-Code
    ...
    while (history_len < in_size) {
        ...
        for (int bit = 0; bit < 8 && history_len < in_size; ++bit) {
            ...
            if (is_match) {
				...
				if (best_match_len == 256) stats256[off_len-1]++;
...
cleanup:
	...
	for (int i=0; i<8; i++)
    fprintf(stderr, "MatchLength:OffsetLength = %s:%s = %s\n", _ui64toaKAZEcomma(256, llTOaDigits2, 10), _ui64toaKAZEcomma(i+1, llTOaDigits3, 10), _ui64toaKAZEcomma(stats256[i], llTOaDigits4, 10));
}
...
int main(int argc, char **argv)
{
    if (argc != 4 && argc != 5 && argc != 6) {
        fprintf(stderr, "lzss_MayMun a.k.a. lzss_MemMem a.k.a. lzss_MaxMin r.%dforte\n", revision);
...
-rw-r--r-- 1 sanmayce sanmayce  70,917,856  'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.MM'        ! r5 !
...
*/
// Note: With r5forte 70,655,686 with (HASH_TABLE_SIZE = 24576 MiB = (HASH_TABLE_SLOTS = 1048576) * (HASH_CANDIDATES = 3072) * (HASH_CANDIDATE_SIZE = 8))
