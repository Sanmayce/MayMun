// lzss_MayMun version 6+ (fixed the mmap so that it really is on file), written by Grok/Gemini AI and Kaze (sanmayce@sanmayce.com), 4003 Lines-Of-Code
// $ gcc -O3 -fopenmp lzss_MayMun.c -o lzss_MayMun.elf -mavx2 -maes
// $ gcc -O3 lzss_MayMun.c -o lzss_MayMun.asm -mavx2 -maes -S
// Note about r6 (when "MIN_MATCH_LEN = 4;"), two lines in CompressMM() deserve poundering, they contain: "MIN_MATCH_LEN*2"

// README.DIZ:
/*
----------------------------------------------------------
LZSS_MAYMUN a.k.a. lzss_MemMem / lzss_MaxMin (Revision 6+)
The MaximumMinimum Granularity-wise Deduplicator
----------------------------------------------------------
Written by: Grok/Gemini AI & Kaze (sanmayce@sanmayce.com)
Date: 08 May 2026
Format: C Source Code (Requires OpenMP, AVX2, AES-NI, POSIX mmap)

------------------------------
★ DESCRIPTION & CORE GOALS ★
------------------------------

MayMun r.6 is a brutalist, bare-metal LZSS compressor designed to 
deduplicate 4++GB datasets with the absolute minimum RAM footprint. 
The goal of MayMun is to decouple compression from the C runtime 
allocator (malloc/free) entirely. By utilizing a 100% POSIX `mmap` 
and `madvise` architecture, MayMun offloads all memory management to 
the OS kernel's virtual page cache. It can seamlessly deduplicate 
files ranging from 1 KB to 16 Exabytes without consuming a single 
byte of heap RAM for buffering.

---------------------------------------------
★ ARCHITECTURAL HIGHLIGHTS & UNIQUENESSES ★
---------------------------------------------

1. THE "RULE OF 8" CPU SYMPATHY (1-Byte Flag Array)
MayMun achieves absurd throughput by front-loading 8 boolean decisions 
into a single uint8_t flag byte. Modern CPUs hate branching; by 
grouping 8 tokens, the CPU processes an entire chunk of data within 
internal registers before touching the disk again. 

2. THE COMPOUND TOKEN (5-Bit Multiplier + 3-Bit Offset Length)
MayMun packs BOTH the variable match length and the variable offset 
length into one byte. This allows the engine to dynamically scale 
match lengths from 8 bytes up to 1,024 bytes in a single hardware-
friendly token.

3. DYNAMIC 64-BIT DICTIONARY (16-Exabyte Sliding Window)
MayMun abandons the 4GB LZ77 limit. Offset lengths scale from 1 to 
8 bytes, allowing backward references across the entire mathematical 
limit of a 64-bit file system.

4. HASH BUCKET MECHANICS (THE SHIFT REGISTER)
To handle hash collisions without the overhead of linked lists or 
dynamic memory allocation, MayMun uses a brutalist Shift Register 
architecture for its hash buckets. 

Each hash index resolves to a contiguous array of `NUM_CANDIDATES`. 
When a new file position is added to a bucket:
1. The array is shifted to the right. 
2. The oldest candidate at the end of the array is overwritten 
   and evicted (Strict FIFO eviction).
3. The new position is injected at index [0].

When the compressor searches a bucket for a match, it iterates 
starting from index [0]. Because the newest data is always at the 
front, MayMun guarantees a "Newest-First" search order, maximizing 
CPU cache locality and prioritizing localized repeating patterns.

5. O(1) MEMORY COMPLEXITY (THE 1x ALLOCATION RULE)
While previous tools like Zirka required up to 50x the input size in 
RAM, MayMun operates in O(1) constant memory time. 
• External RAM requirement: EXACTLY 1x input filesize (allocated 
  via mmap for output).
• Heap usage: Effectively ZERO (no malloc/free for buffers).

6. SSD LONGEVITY (READ-ONLY SOURCE ARCHITECTURE)
MayMun is designed to preserve the lifespan of NAND Flash (SSDs). 
Unlike tools that "thrash" drives with temporary intermediate files 
and heavy write-amplification, MayMun uses a "Read-Once, Write-Once" 
workflow. It maps the input file as Read-Only and streams the 
compressed output linearly, eliminating the write-cycles that 
traditionally destroy SSD health.
Because MayMun uses mmap with PROT_READ on the input,
the kernel treats it as a clean page.
There are no dirty-page flushes to the source,
and the wear-leveling on the SSD stays incredibly low.
...
----------------------------------------------------------
★ THE INVISIBLE RLE: HOW CompressMM() HANDLES OVERLAPS ★
----------------------------------------------------------

LZ77 compression is mathematically a superset of Run-Length Encoding 
(RLE). If a match token dictates a Length that is LARGER than its 
Offset (e.g., Offset: 8, Length: 1024), the decompressor must 
continuously read the bytes it just wrote, cascading a repeating 
pattern forward. 

Most compressors use dedicated branching logic to handle RLE. 
MayMun generates massive RLE tokens completely invisibly using this 
single, emergent C `while` loop over the memory-mapped input:

    // Inside compressMM()
    while (current_match_len < max_possible &&
           input[prev + current_match_len] == 
           input[history_len + current_match_len]) {
        current_match_len++;
    }

Because `input` points to the entire uncompressed file in memory, 
this loop natively feeds into itself without bounds:

EXAMPLE 1: Single-Byte RLE ("AAAAAAAAAA...")
If the file is a solid block of 'A's, and MIN_MATCH_LEN is 8:
• At position 8, the hash table finds the 0th 'A' (offset = 8).
• input[0 + 0] == input[8 + 0] ('A' == 'A')
• input[0 + 1] == input[8 + 1] ('A' == 'A')
The loop continues flawlessly. MayMun outputs a single LZ77 token 
[Offset: 8, Length: 1024] which functions as a pure RLE block.

EXAMPLE 2: Pattern RLE ("ABCABCABCABC...")
If the file is a repeating pattern, MayMun aligns the phase. 
Because of the safety check `(history_len - prev) >= MIN_MATCH_LEN`, 
the offset must be >= 8. 
• At position 9, the hash table finds the 0th 'A' (offset = 9).
  (Note: Offset 9 is perfectly divisible by the 3-byte pattern!)
• input[0 + 0] == input[9 + 0] ('A' == 'A')
• input[0 + 1] == input[9 + 1] ('B' == 'B')
• input[0 + 2] == input[9 + 2] ('C' == 'C')
The loop perfectly maps the phase. MayMun outputs a single LZ77 
token [Offset: 9, Length: 1024] to cascade the "ABC" pattern.

----------------------------------------------------------------------
★ ZERO-RAM DECOMPRESSION (fseek CASCADING) ★
----------------------------------------------------------------------

Decoding overlapping RLE matches on a physical drive would normally 
require thousands of system calls. MayMun's `decompressZeroRAM` uses 
"Mathematically Pure fseek Cascading": it reads the base pattern 
ONCE into a local stack buffer and bulk-writes it forward. This 
replaces thousands of system calls with a single read/write loop 
using exactly 0 bytes of heap RAM.

COMPRESSION MODES:
• 'c' -> Fast FNV-1a 64-bit Hash Mode
• 'C' -> Fast Pippip 128-bit AES Hash Mode
• 'm' -> Exhaustive GLIBC memmem Mode (fixed MatchLength = 8), useful for benchmarking RAM 
• 'M' -> Exhaustive Railgun_Trolldom_64 Mode (fixed MatchLength = 8), useful for benchmarking RAM
...
Examples:
$ ./lzss_MayMun.elf m "$1" "$1".m 8
$ ./lzss_MayMun.elf M "$1" "$1".M 8
$ ./lzss_MayMun.elf d "$1".m "$1".d
$ ./lzss_MayMun.elf d "$1".M "$1".d
$ ./lzss_MayMun.elf C "$1" "$1".C
$ ./lzss_MayMun.elf e "$1".C "$1".e
...
#define HASH_TABLE_SIZE (1ULL << 20) //(1ULL << 17)
#define HASH_MASK (HASH_TABLE_SIZE - 1)
#define NUM_CANDIDATES 3072 //768
//#define NUM_Threads 256
int NUM_Threads;
int GlobalFlagForMemMem=0;

// By default (neither of the two below given) is HASHmalloc:
// -DHASHmmap
// -DHASHmalloc
//#define HASHmmap
//#define HASHmalloc
...
static void show_progress(uint64_t processed, uint64_t total, double mem_mb)
{
    ...
    for (int i = 0; i < bar_width; ++i) {
        if (i < filled) {
        	if (i == filled-1) fputc('#', stderr); else fputc('>', stderr);
		} else {
    	    fputc('.', stderr);
		}
    }
    ...
    //        processed >> 20, total >> 20, (uint64_t)mem_mb);
        if (GlobalFlagForMemMem) {
			fprintf(stderr, "] %d MiB of %d MiB; Memory Footprint: %6.2f",
				    processed >> 20, total >> 20, mem_mb);
		} else {
			fprintf(stderr, "] %d MiB of %d MiB; In/Out = %6.2f",
				    processed >> 20, total >> 20, mem_mb);
		}
    fflush(stderr);
}
...
*/
    ...
    input = mmap(NULL, in_size +256, PROT_READ, MAP_PRIVATE, in_fd, 0); // padding for Pippip, not needed really since there are checks below...
    if (input == MAP_FAILED) {
    ...
    char out_path[] = "/tmp/lzss_MayMun_XXXXXX";
    ...
    size_t hash_size = HASH_TABLE_SIZE * NUM_CANDIDATES * sizeof(uint64_t);
// Why the hell, in r5, this fragment was consuming RAM!?
/*
...
*/
// Therefore in r5forte, replaced with the method used in Zirka:
#if defined(HASHmmap)
    // 2:58:56 (PM9A1 NVMe Samsung 512GB (36308029)) for 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar'
    fprintf(stderr, "Allocating HashTable with mmap...\n"); 
    hash_table = (uint64_t*)create_mmap_file("MayMun_hashtable.tmp", hash_size);
#elif defined(HASHmalloc)
	// 13:14 (PM9A1 NVMe Samsung 512GB (36308029)) for 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar'
    fprintf(stderr, "Allocating HashTable with malloc...\n"); 
    hash_table = malloc(hash_size);
#else
	// by default malloc
    fprintf(stderr, "Allocating HashTable with malloc...\n"); 
    hash_table = malloc(hash_size);
    ...
    while (history_len < in_size) {
        ...
        for (int bit = 0; bit < 8 && history_len < in_size; ++bit) {
            ...
            if (history_len + MIN_MATCH_LEN*2 <= in_size) {
                ...
                for (int c = 0; c < NUM_CANDIDATES; ++c) {
                    ...
                    if (prev != UINT64_MAX && prev < history_len &&
                        (history_len - prev) <= MAX_OFFSET &&
                        (history_len - prev) >= MIN_MATCH_LEN) {

                        int current_match_len = 0;
                        int max_possible = (int)(in_size - history_len);
                        if (max_possible > 32*MIN_MATCH_LEN * 4) max_possible = 32*MIN_MATCH_LEN * 4; // "* 4" because in decompressZeroRAM(): "if (mult==0x1F) mult=((mult+1)<<2)-1; // Add-on since r6, thus 31 becomes 127"
                        ...
                        int quantized = (current_match_len / MIN_MATCH_LEN) * MIN_MATCH_LEN;
                        if (quantized >= MIN_MATCH_LEN && quantized > best_match_len) {
                            best_match_len = quantized;
                            offset = history_len - prev;
                            is_match = true;
                            if (best_match_len == 32*MIN_MATCH_LEN * 4) break; // "* 4" because in decompressZeroRAM(): "if (mult==0x1F) mult=((mult+1)<<2)-1; // Add-on since r6, thus 31 becomes 127"
                        }
                    }
                }
            }
            ...
            if (is_match) {
				...
				int FlagToInflate = 1;
				if (best_match_len == 32*MIN_MATCH_LEN * 4) { stats256[off_len-1]++; FlagToInflate = 4; }
				if ( (best_match_len != 32*MIN_MATCH_LEN * 4) && ((best_match_len > 31*MIN_MATCH_LEN)) ) best_match_len = 31*MIN_MATCH_LEN;
				if (best_match_len == 32*MIN_MATCH_LEN * 4) best_match_len = 32*MIN_MATCH_LEN;

                int mult = (best_match_len / MIN_MATCH_LEN) - 1;
                uint8_t token = (mult << 3) | (off_len - 1);
                ...
                history_len += best_match_len * (FlagToInflate);
            } else {
            ...
            // Update hash table
            while (next_to_hash + MIN_MATCH_LEN*2 <= history_len) {
                uint32_t h = (GlobalFlag_LandonOrPippip == 1) ?
            ...
            if (history_len >= next_progress_update || history_len == in_size) {
                ...
                show_progress(history_len, in_size, (double)history_len/out_pos);
                if (next_progress_update <= in_size)
                    next_progress_update += (1ULL << 20);
            }
        }
...
cleanup:
	...
	for (int i=0; i<8; i++)
    fprintf(stderr, "MatchLength:OffsetLength = %s:%s = %s\n", _ui64toaKAZEcomma(32*MIN_MATCH_LEN * 4, llTOaDigits2, 10), _ui64toaKAZEcomma(i+1, llTOaDigits3, 10), _ui64toaKAZEcomma(stats256[i], llTOaDigits4, 10));
}
...
{
    uint8_t flag;
    uint8_t match_buf[32*MIN_MATCH_LEN * 4]; // MaxMatchLen-byte stack buffer (0 heap RAM used), "* 4" is the inflation of the last/max MatchLen
    ...
    while (fread(&flag, 1, 1, in_fp) == 1) {
        ...
        for (int bit = 0; bit < 8; ++bit) {
            ...
            if (is_match) {
				...
				if (mult==0x1F) mult=((mult+1)<<2)-1; // Add-on since r6, thus 31 becomes 127
                int current_match_len = (mult + 1) * MIN_MATCH_LEN;     // 8..256 bytes
...
int main(int argc, char **argv)
{
    if (argc != 4 && argc != 5 && argc != 6) {
        ...
        fprintf(stderr, "The MaximumMinimum Granularitywise Deduplicator, written by Grok/Gemini AI & Kaze (sanmayce@sanmayce.com)\n");
    ...
    if (argc >= 5)
        ...
        MIN_MATCH_LEN = 8; // 4; //8; // MaximumMinimum
    ...
    if ((use_memmem) || (strcmp(mode, "d") == 0) || (strcmp(mode, "D") == 0))
        ...
        fprintf(stderr, "Deduplicating (32) Granularities = %d,...,%d,(%d)<<2 (at step %d).\n", MIN_MATCH_LEN,31*MIN_MATCH_LEN,32*MIN_MATCH_LEN,MIN_MATCH_LEN);
...
\____|__  (____  / ____\____|__  /____/|___|  /
        \/     \/\/            \/           \/ 
lzss_MayMun a.k.a. lzss_MemMem a.k.a. lzss_MaxMin r.6+
The MaximumMinimum Granularitywise Deduplicator, written by Grok/Gemini AI & Kaze (sanmayce@sanmayce.com)
Deduplicating Granularity = 8.
Using GLIBC memmem().
Compressing 9,154,560 bytes...
Precomputing ALL possible 8-byte matches with 256 threads...
Search Progress = 100.00%; Traversed-So-Far: 12,446,240 MiB (11 TiB); Performance: 28.53 GiB/s
Precompute finished (256-core speedup applied) — starting encoding phase...
Compressing: 100.00% [>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#] 8 MiB of 8 MiB; Memory Footprint: 95.30
...
Compressing 9,154,560 bytes...
Precomputing ALL possible 8-byte matches with 256 threads...
Search Progress = 100.00%; Traversed-So-Far: 12,446,240 MiB (11 TiB); Performance: 28.58 GiB/s
Precompute finished (256-core speedup applied) — starting encoding phase...
Compressing: 100.00% [>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#] 8 MiB of 8 MiB; Memory Footprint: 95.30
Compression complete: 9,154,560 -> 4,883,323 bytes (53.34% of original remains).
...
...
// The 'MayMun' a.k.a. 'Traversing-11-TiB-via-memmem' benchmark is run as superuser, in highest niceness, in Performance Mode. 
// Host: Dell Precision 7560
// CPU: 11th Gen Intel(R) Core(TM) i7-11850H 2x8x1 Threads
// Thread(s) per core: 2
// Core(s) per socket: 8
// Socket(s):          1
// Caches (sum of all):     
...
// Memory: 4x32 GB = 128 GB DDR4, Rank: 2, Configured Memory Speed: 3200 MT/s
...
// The 'MayMun' a.k.a. 'Traversing-11-TiB-via-memmem' benchmark is run as superuser, in highest niceness, in Performance Mode. 
// Host: LENOVO ThinkPad L490
// CPU: i7-8565U 2x4x1 Threads
// Thread(s) per core: 2
// Core(s) per socket: 4
// Socket(s):          1
// Caches (sum of all):     
...
// L3:    8 MiB (1 instance)
// Memory: 2x32 GB = 64 GB DDR4, Rank: 2, Configured Memory Speed: 2400 MT/s
...
// The 'MayMun' a.k.a. 'Traversing-11-TiB-via-memmem' benchmark is run as superuser, in highest niceness, in Performance Mode. 
// Host: LENOVO ThinkPad X270
// CPU: i3-7100U 2x2x1 Threads
// Thread(s) per core: 2
// Core(s) per socket: 2
// Socket(s):          1
// Caches (sum of all):     
...
// L3:    3 MiB (1 instance)
// Memory: 1x32 GB = 32 GB DDR4, Rank: 2, Configured Memory Speed: 2133 MT/s
...
// L3:   16 MiB (1 instance)
// Memory: 4x16 GB = 64 GB LPDDR5, Rank: 2, Configured Memory Speed: 6400 MT/s
Testdatafile: SPETSNAZ_1012_fb2_books_1030-ALL-files.tar, 999,690,240 bytes

+--------------------------------------------------+-------------+----[ Sorted ]-+------------+---------------+
| Compressor / Deduplicator                        | CPU         | Compressed    | Wall clock | Memory        |
|                                                  | utilization | size          | time       | footprint     |
+--------------------------------------------------+-------------+---------------+------------+---------------+
| bwtsatan_static -20e9                            |         99% |   235,772,776 |       2:00 |  6,767,472 KB |
| bsc-3.3.12-x64_CLANG_20.1.8_SSE42 -m0 -e2 -b2047 |        853% |   236,644,378 |       0:26 |  4,883,044 KB |
| zpaq715_GCC_15.2.1_sse4.2 -m511                  |         99% |   237,082,846 |      23:16 | 10,119,204 KB |
| zstd157 --max                                    |         99% |   261,634,849 |      16:36 | 18,297,352 KB |
| rarlinux-x64-712 -s -m5 -md64g -mcx              |       1021% |   265,694,424 |       5:05 |  7,209,716 KB |
| lzss_MayMun_r6 C                                 |         99% |   404,529,708 |    1:52:22 | 26,538,744 KB |
| Zirka_v12++_4096                                 |        837% |   906,965,310 |       1:24 | 47,840,728 KB |
+--------------------------------------------------+-------------+---------------+------------+---------------+
*/
...
TurboBench:  - Wed May  6 11:43:27 2026
   ...
   255304506    25.5       1.39    1415.34     976.54   lzturbo 39      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar  ! Pareto Frontier - Decompression Speed !
   ...
   319488279    32.0      48.70    1571.28     341.29   lzturbo 32      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   342899962    34.3       2.65    1378.12     721.93   lizard 49       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   354152059    35.4       1.58    2827.28     987.77   lzturbo 19      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar  ! Pareto Frontier - Decompression Speed !
   356688238    35.7     329.99    1771.76     360.85   zstd 1d30       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   362786273                                            Nakamichi 'Dragoneye'
   ...
   404529708                                            MayMun r6
   410102166    41.0     395.27    1557.63     413.91   zstd 1          SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   427542587    42.8      41.93    1993.56     452.39   lzturbo 22      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   428456404    42.9       9.35    4822.29     535.78   lzsse2 16       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   438180977    43.8      11.30    5875.55     526.99   lzsse4 16       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar  ! Pareto Frontier - Decompression Speed !
   ...
   454010538    45.4     421.44    2118.98     457.33   lzturbo 30      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   456012566    45.6     135.89    4659.04     463.80   lzturbo 12      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   477293263    47.7     403.84    1434.34     481.16   lizard 40       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   496090296    49.6      83.14    5694.26     508.47   zxc 5           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   524133130    52.4     420.43    2218.97     527.41   lizard 30       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   560165838    56.0     166.34    5904.78     566.51   zxc 4           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar  ! Pareto Frontier - Decompression Speed !
   565630785    56.6     602.64    1995.60     568.29   lzturbo 20      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   579334798    58.0     521.60    2046.82     582.23   lizard 20       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   591751139    59.2     241.74    5931.47     596.22   zxc 3           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   599960599    60.0     653.69    4717.03     601.91   lzturbo 10      SPETSNAZ_1012_fb2_books_1030-ALL-files.tar  ! Pareto Frontier - Compression Speed !
   602118581    60.2     621.29    4291.46     604.19   lz4 0           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   602753876    60.3     555.31    3760.69     605.09   lizard 10       SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   615884187    61.6     496.20    1500.86     619.23   snappy          SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   680866522    68.1     395.28    7894.51     683.65   zxc 2           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   786432540    78.7     603.93    8998.52     788.31   zxc 1           SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
   999690240   100.0   19731.77   20078.13     999.84   memcpy          SPETSNAZ_1012_fb2_books_1030-ALL-files.tar
*/

/*
[sanmayce@djudjeto MayMun6_lzss_Grok_Gemini]$ cat 9.sh 
/bin/time -v ./gzip -f -k -9 "$1"
/bin/time -v ./bwtsatan_static -20e9 "$1" "$1"
/bin/time -v ./zpaq715_GCC_15.2.1_sse4.2.elf a "$1".zpaq "$1" -m511 
/bin/time -v ./bsc-3.3.12-x64_CLANG_20.1.8_SSE42.elf e "$1" "$1".bsc -m0 -e2 -b2047
...
echo
./lzss_MayMun.elf e "$1".MM "$1".restore
echo
b3sum "$1"
b3sum "$1".restore
rm "$1".restore
...
[sanmayce@djudjeto MayMun6_lzss_Grok_Gemini]$ sh 9.sh github.com_mamedev_mame.tar
...
-rw-r--r-- 1 sanmayce sanmayce   317,377,681 May  8 22:47 github.com_mamedev_mame.tar.zpaq
-rw-r--r-- 1 sanmayce sanmayce   345,704,966 May  8 05:08 github.com_mamedev_mame.tar.zstd
-rw-r--r-- 1 sanmayce sanmayce   348,720,895 May  8 23:03 github.com_mamedev_mame.tar.rar
-rw-r--r-- 1 sanmayce sanmayce   352,928,782 May  8 22:47 github.com_mamedev_mame.tar.bsc
-rw-r--r-- 1 sanmayce sanmayce   353,010,696 May  8 22:11 github.com_mamedev_mame.tar.rc
-rw-r--r-- 1 sanmayce sanmayce   393,937,101 May  8 05:08 github.com_mamedev_mame.tar.gz
-rw-r--r-- 1 sanmayce sanmayce   463,427,958 May  8 23:03 github.com_mamedev_mame.tar.MM
-rw-r--r-- 1 sanmayce sanmayce 1,098,076,160 May  8 05:08 github.com_mamedev_mame.tar

[sanmayce@djudjeto MayMun6_lzss_Grok_Gemini]$ sh 9.sh SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_\(2052_HTMs_in_Russian\).tar 
...
-rw-r--r-- 1 sanmayce sanmayce    31,249,772 May  9 00:33 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.zpaq'
-rw-r--r-- 1 sanmayce sanmayce    33,131,092 May  9 00:26 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.rc'
-rw-r--r-- 1 sanmayce sanmayce    34,296,404 May  9 00:33 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.bsc'
-rwxrwxrwx 1 sanmayce sanmayce    42,165,634 Oct 10  2025 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.zstd'
-rw-r--r-- 1 sanmayce sanmayce    43,443,468 May  9 00:38 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.rar'
-rwxrwxrwx 1 sanmayce sanmayce    69,653,162 Oct 10  2025 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.gz'
-rw-r--r-- 1 sanmayce sanmayce    70,644,922 May  9 00:52 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar.MM'
-rwxrwxrwx 1 sanmayce sanmayce   245,732,864 Oct 10  2025 'SUPRAPIG_Marx_Engels_Lenin_Stalin_Collected_Writings_(2052_HTMs_in_Russian).tar'

[sanmayce@djudjeto MayMun6_lzss_Grok_Gemini]$ sh 9.sh Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif 
...
-rw-r--r-- 1 sanmayce sanmayce     5,306,422 May  9 01:02 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.zpaq
-rw-r--r-- 1 sanmayce sanmayce     5,311,582 May  9 01:02 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.bsc
-rw-r--r-- 1 sanmayce sanmayce     5,322,996 May  9 00:57 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.rc
-rw-r--r-- 1 sanmayce sanmayce     6,751,404 Sep  8  2025 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.zstd
-rw-r--r-- 1 sanmayce sanmayce     7,174,345 May  9 01:06 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.rar
-rw-r--r-- 1 sanmayce sanmayce    10,021,974 Sep  8  2025 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.gz
-rw-r--r-- 1 sanmayce sanmayce    10,573,756 May  9 01:13 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif.MM
-rw-r--r-- 1 sanmayce sanmayce   315,527,561 Sep  8  2025 Deathshipcorpus_flat_map_of_the_world_8bit_21792x14479_316MP.tif
*/
